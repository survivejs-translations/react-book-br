...
Eu recomendo você a expandir o aplicativo. A implementação atual deve funcionar como um ponto de partida para algo maior. Além de estender a implementação do arrastar e soltar, você pode tentar adicionar mais dados ao sistema. Você também pode melhorar os estilos. Uma opção seria tentar uma das várias abordagens de estilo discutidas no capítulo *Estilos em React*.

Para deixar sua aplicação ainda mais sustentável durante o desenvolvimento, você também pode implementar testes conforme discutido em *Testando seu código React*. Em *Tipagem em React*, discutimos ainda mais formas de enriquecer seu código. Aprender essas abordagens pode valer a pena. Às vezes, projetar os testes do seu aplicativo em primeiro lugar, ajuda na manutenibilidade. É uma abordagem valiosa, pois permite que você documente sua aplicação à medida que você vai codificando.
